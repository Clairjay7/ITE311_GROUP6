...
<?= $this->section('title') ?>Pending Specimens<?= $this->endSection() ?>
...
<div class="container py-4">
    <div class="page-header">
        <h1><i class="fas fa-vial"></i> Pending Specimens</h1>
    ...
    <div class="modern-card">
        <?php if (empty($pendingSpecimens)): ?>
            <div class="text-center py-5">
                <i class="fas fa-check-circle fa-3x text-success mb-3"></i>
                <p class="lead">No pending specimens found.</p>
            </div>
        <?php else: ?>
            <div class="table-responsive">
                <table class="data-table">
                    ...
                    <tbody>
                        <?php foreach ($pendingSpecimens as $specimen): ?>
                            <tr>
                                <td>#<?= esc($specimen['id']) ?></td>
                                <td>
                                    <strong><?= esc(($specimen['patient_firstname'] ?? '') . ' ' . ($specimen['patient_lastname'] ?? '')) ?></strong>
                                    <?php if (!empty($specimen['patient_contact'])): ?>
                                        <br><small class="text-muted"><?= esc($specimen['patient_contact']) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td><?= esc($specimen['test_type']) ?></td>
                                <td><?= esc($specimen['test_name']) ?></td>
                                <td>
                                    <?php if ($specimen['requested_by'] === 'doctor' && !empty($specimen['doctor_name'])): ?>
                                        <i class="fas fa-user-md"></i> Dr. <?= esc($specimen['doctor_name']) ?>
                                    <?php elseif ($specimen['requested_by'] === 'nurse' && !empty($specimen['nurse_name'])): ?>
                                        <i class="fas fa-user-nurse"></i> <?= esc($specimen['nurse_name']) ?>
                                    <?php else: ?>
                                        <?= esc(ucfirst($specimen['requested_by'])) ?>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <span class="badge badge-<?= esc($specimen['priority']) ?>">
                                        <?= esc(ucfirst($specimen['priority'])) ?>
                                    </span>
                                </td>
                                <td><?= esc(date('M d, Y', strtotime($specimen['requested_date']))) ?></td>
                                <td>
                                    <span class="badge badge-<?= esc(str_replace('_', '-', $specimen['status'])) ?>">
                                        <?= esc(ucfirst(str_replace('_', ' ', $specimen['status']))) ?>
                                    </span>
                                </td>
                                <td>
...
function markCollected(requestId) {
    ...
    fetch('<?= site_url('labstaff/pending-specimens/mark-collected') ?>/' + requestId, {
        method: 'POST',
        headers: {
...
document.getElementById('completeForm').addEventListener('submit', function(e) {
    ...
    fetch('<?= site_url('labstaff/pending-specimens/mark-completed') ?>/' + requestId, {
        method: 'POST',
        body: formData,
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
