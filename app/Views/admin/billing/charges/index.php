...
<div class="status-filter">
    <a href="<?= base_url('admin/billing/charges') ?>" 
       class="filter-btn <?= $currentStatus === 'all' ? 'active' : '' ?>">
        All (<?= count($charges) ?>)
    </a>
    <a href="<?= base_url('admin/billing/charges?status=pending') ?>" 
       class="filter-btn <?= $currentStatus === 'pending' ? 'active' : '' ?>">
        Pending
    </a>
    <a href="<?= base_url('admin/billing/charges?status=approved') ?>" 
       class="filter-btn <?= $currentStatus === 'approved' ? 'active' : '' ?>">
        Approved
    </a>
    <a href="<?= base_url('admin/billing/charges?status=paid') ?>" 
       class="filter-btn <?= $currentStatus === 'paid' ? 'active' : '' ?>">
        Paid
    </a>
    <a href="<?= base_url('admin/billing/charges?status=cancelled') ?>" 
            ...
            <?php if (!empty($charges)): ?>
                <?php foreach ($charges as $charge): ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?= base_url('admin/billing/charges/view/' . $charge['id']) ?>" 
                               class="btn-action btn-view">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <?php if ($charge['status'] === 'paid'): ?>
                                <a href="<?= base_url('admin/billing/charges/invoice/' . $charge['id']) ?>" 
...
async function approveCharge(chargeId) {
    ...
    try {
        const response = await fetch(`<?= base_url('admin/billing/charges/approve/') ?>${chargeId}`, {
            method: 'POST',
            headers: {
...
async function processPayment(chargeId) {
    ...
    try {
        ...
        const response = await fetch(`<?= base_url('admin/billing/charges/process-payment/') ?>${chargeId}`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
        ...
        if (data.success) {
            ...
            if (confirm(message + '\n\nDo you want to print the receipt now?')) {
                window.open(`<?= base_url('admin/billing/charges/invoice/') ?>${chargeId}`, '_blank');
            }
            location.reload();
        } else {
...
async function cancelCharge(chargeId) {
    ...
    try {
        const response = await fetch(`<?= base_url('admin/billing/charges/cancel/') ?>${chargeId}`, {
            method: 'POST',
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
