...
<div class="admin-module">
    ...
    <form method="POST" action="<?= base_url('admin/billing/update/' . $billing['id']) ?>" class="form-container">
        <div class="form-group">
            <label for="patient_id">Patient *</label>
            <select id="patient_id" name="patient_id" class="form-control" required>
                <option value="">Select Patient</option>
                <?php foreach ($patients as $patient): ?>
                    <option value="<?= esc($patient['id']) ?>" <?= old('patient_id', $billing['patient_id']) == $patient['id'] ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="service" name="service" class="form-control" value="<?= old('service', $billing['service']) ?>" required>
        </div>

        <div class="form-group">
            <label for="amount">Amount *</label>
            <input type="number" id="amount" name="amount" step="0.01" class="form-control" value="<?= old('amount', $billing['amount']) ?>" required>
        </div>
