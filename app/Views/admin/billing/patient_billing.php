...
<div class="search-container">
    <form method="GET" action="<?= base_url('admin/billing/patient_billing') ?>">
        ...
        <form id="paymentForm" onsubmit="processPayment(event)">
            ...
            <?php if (!empty($availableInsurances)): ?>
                ...
                <div id="insurance_selection" style="display: none; margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Select Insurance Provider <span style="color: red;">*</span></label>
                    <select name="insurance_provider" id="insurance_provider_select" class="form-control" required onchange="calculateInsuranceCoverage(); updateActiveInsurance()">
                    ...
                    <small style="color: #64748b; font-size: 12px; margin-top: 4px; display: block;">
                        <i class="fas fa-info-circle"></i> Ang deduction ay auto-compute base sa selected insurance provider
                    </small>
                </div>
...
function openPaymentModal() {
    ...
    currentBillType = billTypes.split(',')[0]; // Use first type as default
...
function updateInsuranceFields() {
    ...
    if (paymentMethod === 'insurance' && insuranceSelection) {
        ...
        if (insuranceSelection) insuranceSelection.style.display = 'none';
        if (insuranceCoverage) insuranceCoverage.style.display = 'none';
        if (document.getElementById('insurance_provider_select')) {
...
function calculateInsuranceCoverage() {
    ...
    if (!insuranceProvider || !insuranceCoverage || currentBillAmount <= 0) {
        if (insuranceCoverage) insuranceCoverage.style.display = 'none';
...
async function processPayment(event) {
    ...
    try {
        const response = await fetch('<?= base_url('admin/billing/process-bill-payment') ?>', {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize insurance limits display
    updateActiveInsurance();
});
