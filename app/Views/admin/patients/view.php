...
<div class="patient-view container py-4">
    ...
    <div class="card-body">
      ...
      <div class="row g-3 mb-3">
        <div class="col-md-3">
          ...
          <div class="fw-semibold">#<?= esc($patient['patient_id'] ?? $patient['id']) ?></div>
        </div>
      ...
      <div class="row g-3 mb-3">
        ...
        <?php endif; ?>
        <?php if (hasValue($patient['doctor_full_name'] ?? $patient['doctor_name'] ?? '')): ?>
        <div class="col-md-3 field-row has-value">
          <div class="text-muted">Doctor Assigned</div>
          <div class="fw-semibold"><?= esc($patient['doctor_full_name'] ?? $patient['doctor_name']) ?></div>
        </div>
        <?php endif; ?>
      ...
      <!-- F. In-Patient Details -->
      <?php if (($patient['type'] ?? '') === 'In-Patient'): ?>
      <h6 class="section-title mb-2">F. In-Patient Details</h6>
      <div class="row g-3 mb-3">
        ...
        <?php endif; ?>
        <?php if (hasValue($patient['admission_time'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Admission Time</div>
          <div class="fw-semibold"><?= esc($patient['admission_time']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($patient['room_type'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Room Type</div>
          <div class="fw-semibold"><?= esc($patient['room_type']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($patient['room_num'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Room Number</div>
          <div class="fw-semibold"><?= esc($patient['room_num']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($bed['bed_number'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Bed Number</div>
          <div class="fw-semibold"><?= esc($bed['bed_number']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($patient['admission_reason'] ?? '')): ?>
        <div class="col-12 field-row has-value">
          <div class="text-muted">Admission Reason</div>
          <div class="fw-semibold"><?= nl2br(esc($patient['admission_reason'])) ?></div>
        </div>
        <?php endif; ?>
      </div>
      <?php endif; ?>

      <!-- G. Out-Patient Details -->
      <?php if (($patient['type'] ?? '') === 'Out-Patient'): ?>
      <h6 class="section-title mb-2">G. Out-Patient Details</h6>
      <div class="row g-3 mb-3">
        <?php if (hasValue($patient['visit_date'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Visit Date</div>
          <div class="fw-semibold"><?= esc($patient['visit_date']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($patient['visit_time'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Visit Time</div>
          <div class="fw-semibold"><?= esc($patient['visit_time']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($consultation['consultation_date'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Consultation Date</div>
          <div class="fw-semibold"><?= esc($consultation['consultation_date']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($consultation['consultation_time'] ?? '')): ?>
        <div class="col-md-4 field-row has-value">
          <div class="text-muted">Consultation Time</div>
          <div class="fw-semibold"><?= esc($consultation['consultation_time']) ?></div>
        </div>
        <?php endif; ?>
        <?php if (hasValue($patient['purpose'] ?? '')): ?>
        <div class="col-12 field-row has-value">
  ...
  <div class="action-bar text-center mt-3">
    <a class="btn btn-outline-secondary me-2" href="<?= site_url('admin/patients') ?>">Back</a>
    <a class="btn btn-primary" href="<?= site_url('admin/patients/edit/'.esc($patient['patient_id'] ?? $patient['id'])) ?>">Edit</a>
  </div>
</div>
<?= $this->endSection() ?>
