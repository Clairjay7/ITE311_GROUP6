            ...
            <div class="form-section" style="margin-bottom: 32px;">
                ...
                <div class="form-row">
                    <div class="form-group full-width">
                        ...
                        <select name="type" id="patient_type" class="form-select" required disabled style="background: #f3f4f6; cursor: not-allowed;">
                            <option value="">-- Select Patient Type --</option>
                            <option value="In-Patient" <?= set_select('type', 'In-Patient', ($patient['type'] ?? '') === 'In-Patient') ?> <?= ($patient['type'] ?? '') === 'In-Patient' ? 'selected' : '' ?>>In-Patient (Hospital Admission)</option>
                            <option value="Out-Patient" <?= set_select('type', 'Out-Patient', ($patient['type'] ?? '') === 'Out-Patient') ?> <?= ($patient['type'] ?? '') === 'Out-Patient' ? 'selected' : '' ?>>Out-Patient (Consultation/Check-up)</option>
                        </select>
                        <div class="form-hint">Patient type cannot be changed during edit</div>
                    </div>
                </div>
            </div>
            
            <form method="post" action="<?= base_url('admin/patients/update/' . ($patient['patient_id'] ?? $patient['id'])) ?>" id="patientForm">
                <?= csrf_field() ?>
                <input type="hidden" name="type" id="hidden_type" value="<?= set_value('type', $patient['type'] ?? 'In-Patient') ?>">
                <input type="hidden" name="visit_type" id="hidden_visit_type" value="<?= set_value('visit_type', $patient['visit_type'] ?? 'Consultation') ?>">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="first_name" class="form-control <?= isset($errors['first_name']) ? 'is-invalid' : '' ?>" 
                                   value="<?= set_value('first_name', $patient['first_name'] ?? '') ?>" required placeholder="Enter first name">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="middle_name" class="form-control" 
                                   value="<?= set_value('middle_name', $patient['middle_name'] ?? '') ?>" placeholder="Optional">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="last_name" class="form-control <?= isset($errors['last_name']) ? 'is-invalid' : '' ?>" 
                                   value="<?= set_value('last_name', $patient['last_name'] ?? '') ?>" required placeholder="Enter surname">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="age" id="age" class="form-control" 
                                   value="<?= set_value('age', $patient['age'] ?? '') ?>" readonly placeholder="Auto-calculated">
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="contact" class="form-control" 
                                   value="<?= set_value('contact', $patient['contact'] ?? '') ?>" required placeholder="09XX-XXX-XXXX">
                        </div>
                        
                        <div class="form-group" style="grid-column: span 2;">
                            <label class="form-label">Complete Address <span class="required">*</span></label>
                            <input type="text" name="address" class="form-control" 
                                   value="<?= set_value('address', $patient['address'] ?? '') ?>" required placeholder="House No., Street, Barangay, City/Municipality, Province">
                        </div>
                    </div>
                </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-row">
                            ...
                            <select name="doctor_id" id="admitting_doctor" class="form-select" required>
                                ...
                                <?php if (!empty($doctors)): ?>
                                    <?php foreach ($doctors as $doctor): ?>
                                        <option value="<?= esc($doctor['id']) ?>" <?= set_select('doctor_id', $doctor['id'], ($patient['doctor_id'] ?? null) == $doctor['id']) ?> data-specialization="<?= esc(strtolower($doctor['specialization'] ?? '')) ?>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="admission_date" class="form-control <?= isset($errors['admission_date']) ? 'is-invalid' : '' ?>" 
                                   value="<?= set_value('admission_date', $patient['admission_date'] ?? date('Y-m-d')) ?>" 
                    ...
                    <div class="form-row">
                        <div class="form-group full-width">
                            ...
                            <textarea name="purpose" class="form-control" rows="3" required 
                                      placeholder="Enter the reason for admission or chief complaint"><?= set_value('purpose', $patient['purpose'] ?? '') ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="room_type" id="room_type" class="form-select" required>
                                ...
                                <?php
                                // Get room type from patient's current room
                                $currentRoomType = '';
                                if (!empty($patient['room_id'])) {
                                    $db = \Config\Database::connect();
                                    $currentRoom = $db->table('rooms')->where('id', $patient['room_id'])->get()->getRowArray();
                                    $currentRoomType = $currentRoom['room_type'] ?? '';
                                }
                                ?>
                                <option value="Private" <?= set_select('room_type', 'Private', $currentRoomType === 'Private') ?>>Private Room - ₱5,000/day (1 patient, own CR)</option>
                                <option value="Semi-Private" <?= set_select('room_type', 'Semi-Private', $currentRoomType === 'Semi-Private') ?>>Semi-Private Room - ₱3,000/day (2 patients, shared CR)</option>
                                <option value="Ward" <?= set_select('room_type', 'Ward', $currentRoomType === 'Ward' || empty($currentRoomType)) ?>>Ward / General Ward - ₱1,000/day (4-10+ patients, shared facilities)</option>
                                <option value="ICU" <?= set_select('room_type', 'ICU', $currentRoomType === 'ICU') ?>>ICU (Intensive Care Unit) - ₱8,000/day (Critical care, special equipment)</option>
                                <option value="Isolation" <?= set_select('room_type', 'Isolation', $currentRoomType === 'Isolation') ?>>Isolation Room - ₱6,000/day (For infectious diseases, separate ventilation)</option>
                                <option value="NICU" <?= set_select('room_type', 'NICU', $currentRoomType === 'NICU') ?> data-age-limit="28">NICU (Neonatal Intensive Care Unit) - ₱10,000/day (For newborns 0-28 days old only)</option>
                            </select>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="room_id" id="room_id" value="<?= esc($patient['room_id'] ?? '') ?>">
                        ...
                        <div class="form-group">
                            ...
                            if (!empty($patient['room_id'])) {
                                ...
                                if ($db->tableExists('beds')) {
                                    $currentBed = $db->table('beds')
                                        ->where('room_id', $patient['room_id'])
                                        ->where('current_patient_id', $patient['patient_id'] ?? $patient['id'])
                                        ->get()
                                        ->getRowArray();
                                    if ($currentBed) {
                                        $currentBedId = $currentBed['id'];
                                        $currentBedNumber = $currentBed['bed_number'];
                                    }
                                }
                            }
                            ?>
                            <input type="hidden" name="bed_id" id="bed_id" value="<?= esc($currentBedId ?? '') ?>">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group" style="margin-bottom: 20px;">
                        ...
                        <?php
                        $currentVisitType = $patient['visit_type'] ?? 'Consultation';
                        ?>
                        <div class="visit-type-options" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px;">
                            <label class="visit-type-card" style="position: relative; padding: 16px; border: 2px solid #e5e7eb; border-radius: 12px; cursor: pointer; text-align: center; transition: all 0.2s ease;">
                                <input type="radio" name="visit_type" value="Consultation" <?= set_radio('visit_type', 'Consultation', $currentVisitType === 'Consultation') ?> required>
                            ...
                            <label class="visit-type-card" style="position: relative; padding: 16px; border: 2px solid #e5e7eb; border-radius: 12px; cursor: pointer; text-align: center; transition: all 0.2s ease;">
                                <input type="radio" name="visit_type" value="Follow-up" <?= set_radio('visit_type', 'Follow-up', $currentVisitType === 'Follow-up') ?>>
                            ...
                            <label class="visit-type-card" style="position: relative; padding: 16px; border: 2px solid #e5e7eb; border-radius: 12px; cursor: pointer; text-align: center; transition: all 0.2s ease;">
                                <input type="radio" name="visit_type" value="Check-up" <?= set_radio('visit_type', 'Check-up', $currentVisitType === 'Check-up') ?>>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="doctor_id" id="outpatient_doctor_id" class="form-select" required>
                                ...
                                <?php if (!empty($doctors)): ?>
                                    <?php foreach ($doctors as $doctor): ?>
                                        <option value="<?= esc($doctor['id']) ?>" <?= set_select('doctor_id', (string)$doctor['id'], ($patient['doctor_id'] ?? null) == $doctor['id']) ?> data-specialization="<?= esc(strtolower($doctor['specialization'] ?? '')) ?>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="appointment_date" id="appointment_date" class="form-control" 
                                   value="<?= set_value('appointment_date', $patient['appointment_date'] ?? date('Y-m-d')) ?>" 
                                   min="<?= date('Y-m-d') ?>" required>
                    ...
                    <div class="form-row">
                        <div class="form-group full-width">
                            ...
                            <textarea name="purpose" class="form-control" rows="3" required 
                                      placeholder="Briefly describe the patient's primary concern or purpose"><?= set_value('purpose', $patient['purpose'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
                </div>
                    ...
                    <div class="form-row">
                        <div class="form-group full-width">
                            ...
                            <textarea name="existing_conditions" class="form-control" rows="2" 
                                      placeholder="e.g., Diabetes, Hypertension, Asthma (Optional)"><?= set_value('existing_conditions', $patient['existing_conditions'] ?? '') ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group full-width">
                            ...
                            <textarea name="allergies" class="form-control" rows="2" 
                                      placeholder="e.g., Penicillin, Seafood, Latex (Optional)"><?= set_value('allergies', $patient['allergies'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
                </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="insurance_number" class="form-control" id="insurance_number"
                                       value="<?= set_value('insurance_number', $patient['insurance_number'] ?? '') ?>" placeholder="Enter insurance/member ID">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="emergency_name" class="form-control" 
                                   value="<?= set_value('emergency_name', $patient['emergency_name'] ?? '') ?>" required placeholder="Full name of contact person">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="emergency_relationship" class="form-select" required>
                                ...
                                <option value="Spouse" <?= set_select('emergency_relationship', 'Spouse', ($patient['emergency_relationship'] ?? '') === 'Spouse') ?>>Spouse</option>
                                <option value="Parent" <?= set_select('emergency_relationship', 'Parent', ($patient['emergency_relationship'] ?? '') === 'Parent') ?>>Parent</option>
                                <option value="Child" <?= set_select('emergency_relationship', 'Child', ($patient['emergency_relationship'] ?? '') === 'Child') ?>>Child</option>
                                <option value="Sibling" <?= set_select('emergency_relationship', 'Sibling', ($patient['emergency_relationship'] ?? '') === 'Sibling') ?>>Sibling</option>
                                <option value="Relative" <?= set_select('emergency_relationship', 'Relative', ($patient['emergency_relationship'] ?? '') === 'Relative') ?>>Relative</option>
                                <option value="Friend" <?= set_select('emergency_relationship', 'Friend', ($patient['emergency_relationship'] ?? '') === 'Friend') ?>>Friend</option>
                                <option value="Guardian" <?= set_select('emergency_relationship', 'Guardian', ($patient['emergency_relationship'] ?? '') === 'Guardian') ?>>Guardian</option>
                                <option value="Other" <?= set_select('emergency_relationship', 'Other', ($patient['emergency_relationship'] ?? '') === 'Other') ?>>Other</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="emergency_contact" class="form-control" 
                                   value="<?= set_value('emergency_contact', $patient['emergency_contact'] ?? '') ?>" required placeholder="09XX-XXX-XXXX">
                        </div>
                    </div>
                </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (patientTypeSelect && hiddenTypeInput) {
        ...
        // Initialize on page load - use patient's current type
        const currentPatientType = patientTypeSelect ? patientTypeSelect.value : '';
        if (currentPatientType) {
            updateFormForType(currentPatientType);
        } else {
            // Default to In-Patient if not set
    ...
    // Initialize form with patient data on page load
    function initializeEditForm() {
        // Get current patient type and set form accordingly
        const currentType = patientTypeSelect ? patientTypeSelect.value : '';
        if (currentType) {
            updateFormForType(currentType);
        }
        
        // Initialize room and bed for In-Patient
        if (currentType === 'In-Patient') {
            ...
            // Get current room_id and room_number from patient data
            const currentRoomId = roomIdField ? roomIdField.value : '';
            const currentRoomNumber = '<?= esc($patient['room_number'] ?? '') ?>';
            const currentBedId = bedIdField ? bedIdField.value : '';
            const currentBedNumber = '<?= esc($currentBedNumber ?? '') ?>';
            
            // If patient has a room, populate room type and room number
            if (currentRoomId && roomTypeSelect && roomNumberSelect) {
                // Get room type from current room
                const roomsDataElement = document.getElementById('rooms-data');
                if (roomsDataElement) {
                    try {
                        const roomsData = JSON.parse(roomsDataElement.textContent);
                        let foundRoomType = '';
                        
                        // Find which room type contains this room
                        for (const roomType in roomsData) {
                            const room = roomsData[roomType].find(r => (r.id || r.room_id) == currentRoomId);
                            if (room) {
                        ...
                        // Also try to get from database via PHP
                        <?php if (!empty($patient['room_id'])): ?>
                            <?php
                            $db = \Config\Database::connect();
                            $currentRoomInfo = $db->table('rooms')->where('id', $patient['room_id'])->get()->getRowArray();
                            $currentRoomTypeFromDB = $currentRoomInfo['room_type'] ?? '';
                            ?>
                            const roomTypeFromDB = '<?= esc($currentRoomTypeFromDB) ?>';
                            if (roomTypeFromDB) {
                                foundRoomType = roomTypeFromDB;
                            }
                        <?php endif; ?>
                        
                        if (foundRoomType) {
                            roomTypeSelect.value = foundRoomType;
                            // Trigger change to populate room dropdown
                            roomTypeSelect.dispatchEvent(new Event('change'));
                            
                            // Wait a bit for room dropdown to populate, then select current room
                            setTimeout(function() {
                                if (roomNumberSelect && currentRoomNumber) {
                                    // Find and select the current room
                                    for (let i = 0; i < roomNumberSelect.options.length; i++) {
                                        const option = roomNumberSelect.options[i];
                                        if (option.value === currentRoomNumber || option.dataset.roomId == currentRoomId) {
                                            roomNumberSelect.value = option.value;
                        ...
                        console.error('Error initializing rooms:', e);
                    }
                }
            }
            
            // Initialize bed if patient has one
            if (currentBedId && bedNumberSelect) {
                setTimeout(function() {
                    for (let i = 0; i < bedNumberSelect.options.length; i++) {
                        const option = bedNumberSelect.options[i];
                        if (option.dataset.bedId == currentBedId || option.value === currentBedNumber) {
                            bedNumberSelect.value = option.value;
                            bedNumberSelect.dispatchEvent(new Event('change'));
                            break;
                        }
                    }
                }, 1000);
        ...
        // Initialize appointment date/time for Out-Patient
        if (currentType === 'Out-Patient') {
            const appointmentDateInput = document.getElementById('appointment_date');
            const appointmentTimeInput = document.getElementById('appointment_time');
            
            // Get appointment date from patient data or consultation
            const patientAppointmentDate = '<?= esc($patient['appointment_date'] ?? '') ?>';
            if (patientAppointmentDate && appointmentDateInput) {
                appointmentDateInput.value = patientAppointmentDate;
            }
            
            // Get appointment time from consultation if available
            <?php
            $appointmentTime = '';
            if (!empty($patient['patient_id'] ?? $patient['id'])) {
                $db = \Config\Database::connect();
                if ($db->tableExists('consultations')) {
                    $consultation = $db->table('consultations')
                        ->where('patient_id', $patient['patient_id'] ?? $patient['id'])
                        ->orderBy('consultation_date', 'DESC')
                        ->get()
                        ->getRowArray();
                    if ($consultation && !empty($consultation['consultation_time'])) {
                        // Extract time from datetime (HH:MM:SS format)
                        $time = $consultation['consultation_time'];
                        if (strlen($time) >= 5) {
                            $appointmentTime = substr($time, 0, 5); // Get HH:MM
                        }
                    }
                }
            }
            ?>
            const patientAppointmentTime = '<?= esc($appointmentTime) ?>';
            if (patientAppointmentTime && appointmentTimeInput) {
                appointmentTimeInput.value = patientAppointmentTime;
            }
        }
    }
    ...
    function initializeRoomPrices() {
        ...
        // Initialize edit form with patient data after a delay
        setTimeout(function() {
