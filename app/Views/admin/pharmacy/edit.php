...
<div class="admin-module">
    ...
    <form method="POST" action="<?= base_url('admin/pharmacy/update/' . $pharmacyItem['id']) ?>" class="form-container">
        <div class="form-group">
            ...
            <input type="text" id="item_name" name="item_name" class="form-control" value="<?= old('item_name', $pharmacyItem['item_name']) ?>" required>
        </div>

        <div class="form-group">
            <label for="description">Description</label>
            <textarea id="description" name="description" class="form-control" rows="3"><?= old('description', $pharmacyItem['description']) ?></textarea>
        </div>

        <div class="form-group">
            <label for="quantity">Quantity *</label>
            <input type="number" id="quantity" name="quantity" min="0" class="form-control" value="<?= old('quantity', $pharmacyItem['quantity']) ?>" required>
        </div>

        <div class="form-group">
            <label for="price">Price *</label>
            <input type="number" id="price" name="price" step="0.01" min="0" class="form-control" value="<?= old('price', $pharmacyItem['price']) ?>" required>
        </div>
