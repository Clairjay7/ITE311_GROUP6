...
<div class="admin-page-container">
    ...
    <div class="modern-card">
        <form action="<?= site_url('admin/users/update/' . $user['id']) ?>" method="post">
            ...
            <div class="form-group-modern" id="shift_preference_group" style="display: <?= ($nurseRoleId && $user['role_id'] == $nurseRoleId) ? 'block' : 'none'; ?>;">
                <label class="form-label-modern" for="shift_preference">
                    <i class="fas fa-clock me-2"></i>
                    Shift Preference <span class="text-danger">*</span>
                </label>
                <select name="shift_preference" id="shift_preference" class="form-control-modern" <?= ($nurseRoleId && $user['role_id'] == $nurseRoleId) ? 'required' : '' ?>>
                    <option value="">Select Shift Preference</option>
                    <option value="morning" <?= (old('shift_preference', $currentShiftPreference) == 'morning') ? 'selected' : '' ?>>Morning Shift</option>
                    <option value="night" <?= (old('shift_preference', $currentShiftPreference) == 'night') ? 'selected' : '' ?>>Night Shift</option>
                    <option value="bulk" <?= (old('shift_preference', $currentShiftPreference) == 'bulk') ? 'selected' : '' ?>>Bulk (Both Shifts)</option>
                </select>
